...
def logistic_predict(weights, data):
    ...
    # y is the probabilities being second class.
    bias = weights[-1]
    dot_product = np.dot(data, weights[:-1]) + bias
    y = sigmoid(-dot_product)
...
def evaluate(targets, y):
    ...
    frac_correct = float(correct_count)/(correct_count + false_count)
...
def logistic(weights, data, targets, hyperparameters):
    ...
    if hyperparameters['weight_regularization'] is True:
        ...
        df = np.zeros(shape = (len(weights), 1))

        z = np.dot(data, weights[:-1]) + weights[-1]
        f = np.sum(targets * z + np.log(1+ np.exp(-z)))

        df[:-1] = np.dot(data.T , (targets - y))
        df[-1] = np.sum(targets - y)
...
def logistic_pen(weights, data, targets, hyperparameters):
    ...
    z = np.dot(data, weights[:-1]) + weights[-1]
    f = np.sum(targets * z + np.log(1 + np.exp(-z)) ) + 0.5 * hyperparameters['weight_decay'] * pow(np.sum(weights),2)

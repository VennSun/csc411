...
def q2():
    ...
    ShowMeans(mu, 1)
    ShowMeans(vary, 2)
    print 'probabilities of clusters = %s' % p


def error_rate(t, error_rate, p1, p2, mu1, mu2, vary1, vary2, input, label, log_likelihood_class):
    logLikelihood_1 = mogLogLikelihood(p1, mu1, vary1, input) + log_likelihood_class[0]
    logLikelihood_2 = mogLogLikelihood(p2, mu2, vary2, input) + log_likelihood_class[1]

    predict = (logLikelihood_1 < logLikelihood_2).astype(float)

    error_rate[t] = np.sum((predict != label).astype(float)) / label.shape[0]
...
def q4():
    ...
    for t in xrange(T):
        ...
        Anger_p, Anger_mu, Anger_vary, Anger_logLikelihood = mogEM(x_train_anger, K, iters, randConst, minVary)

        Happy_p, Happy_mu, Happy_vary, Happy_logLikelihood = mogEM(x_train_happy, K, iters, randConst, minVary)
        ...
        #-------------------- Add your code here ------------------------------
        # Training set
        errorTrain[t] = error_rate(t,errorTrain, Anger_p, Happy_p, Anger_mu, Happy_mu, Anger_vary, Happy_vary,
                                 x_train, y_train, log_likelihood_class)
        print 'Training error rate : %.5f' % errorTrain[t]

        # validation set
        errorValidation[t] = error_rate(t, errorValidation, Anger_p, Happy_p, Anger_mu, Happy_mu, Anger_vary, Happy_vary,
                                 x_valid, y_valid, log_likelihood_class)
        print 'Validation error rate : %.5f' % errorValidation[t]

        # test set
        errorTest[t] = error_rate(t, errorTest, Anger_p, Happy_p, Anger_mu, Happy_mu, Anger_vary, Happy_vary,
                                 x_test, y_test, log_likelihood_class)
        print 'Test error rate : %.5f' % errorTest[t]
